...
function getWindowTitle(windowId) {
    ...
    const titles = {
        'game': isEn ? "Nine Men's Morris" : 'Młynek',
        'about': isEn ? 'About' : 'O grze',
        'projects': isEn ? 'My Projects' : 'Moje projekty',
        'calculator': isEn ? 'Calculator' : 'Kalkulator',
        'themes': isEn ? 'Display Properties' : 'Właściwości: Ekran',
        'notepad': isEn ? 'Notepad' : 'Notatnik',
        'gameover': isEn ? 'Game Over' : 'Koniec gry'
    };
...
function showShutdownDialog() {
    ...
    const isEn = document.documentElement.lang === 'en';
    alert(isEn 
        ? 'Thank you for visiting my Windows 98 style site!\n\nThis is just a demo shutdown window.' 
        : 'Dziękujemy za odwiedzenie mojej strony w stylu Windows 98!\n\nTo jest tylko demonstracyjne okno zamykania systemu.');
}
...
function initDragAndDrop() {
    ...
    document.addEventListener('mousemove', function (e) {
        if (draggedElement) {
            ...
            // Calculate relative to desktop
            const x = (e.clientX - desktopRect.left) - offset.x;
            const y = (e.clientY - desktopRect.top) - offset.y;

            const maxX = desktopRect.width - draggedElement.offsetWidth;
            // 28px taskbar offset
            const maxY = desktopRect.height - draggedElement.offsetHeight - 28;
...
function applyCurrentTheme() {
    ...
    const isEn = document.documentElement.lang === 'en';
    alert(isEn ? 'Theme applied!' : 'Motyw został zastosowany!');
}
